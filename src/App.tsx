...
export default function App() {
  ...
  const renderCurrentView = () => {
    ...
    if (path.startsWith('/team')) {
      let subRoute: 'faculty' | 'coordinators' | 'technical-leads' | 'alumni' | undefined;
      if (path.includes('/faculty')) subRoute = 'faculty';
      else if (path.includes('/coordinators')) subRoute = 'coordinators';
      else if (path.includes('/technical-leads')) subRoute = 'technical-leads';
      else if (path.includes('/alumni')) subRoute = 'alumni';
    ...
    // Dynamic Member Profile Route (/profile or /profile/:idOrUsername)
    if (path.startsWith('/profile')) {
      const parts = path.split('/').filter(Boolean);
      const targetUserIdOrUsername = parts.length > 1 ? parts[1] : undefined;

      return (
        <ProfileView
          currentUser={user}
          targetUserIdOrUsername={targetUserIdOrUsername}
          navigate={navigate}
          projects={projects}
          events={events}
