...
export const AdminDashboardView: React.FC<AdminDashboardViewProps> = ({
  ...
  const [activeSection, setActiveSection] = useState<
    'overview' | 'applications' | 'registrations' | 'users' | 'faculty' | 'coordinators' | 'projects' | 'events' | 'blog' | 'messages' | 'logs' | 'settings'
  >('overview');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8 text-slate-100">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-5 text-xs">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
            <div>
              ...
              <p className="text-slate-400 text-xs">Promote members, assign faculty coordinators, update security roles, and monitor registered account statuses.</p>
            </div>
      ...
      {activeSection === 'faculty' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 text-xs">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="font-bold text-white text-base">Faculty Coordinators Directory CRUD</h3>
            <button
            ...
            {faculty.map(f => (
              <div key={f.id} className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400">{f.designation} • {f.department}</p>
                </div>
                <button
      ...
      {activeSection === 'coordinators' && (
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-4 text-xs">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="font-bold text-white text-base">Student Coordinators Management</h3>
            <button
            ...
            {coordinators.map(c => (
              <div key={c.id} className="p-4 rounded-xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-400">{c.position} • {c.branch} ({c.year})</p>
                </div>
                <button
      ...
      {showFacultyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl max-w-md w-full p-6 space-y-4 text-slate-100 text-xs">
            <h3 className="font-bold text-white text-base">Add New Faculty Coordinator</h3>
