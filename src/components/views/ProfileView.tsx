...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      try {
        const data = await safeFetchJson(`/api/users/public/${encodeURIComponent(targetUserIdOrUsername)}`);
        if (data) {
          setProfileUser(data);
          syncEditForm(data);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await safeParseResponse(res);
      if (!res.ok || error || !data) {
        throw new Error(data?.error || error || 'Failed to update profile.');
      }
