...
export const HomeView: React.FC<HomeViewProps> = ({
  ...
  return (
    <div className="space-y-16 sm:space-y-24 pb-16 text-slate-100">
              ...
              <div className="relative rounded-2xl bg-gradient-to-b from-slate-900 to-slate-950 border border-slate-800 p-2 shadow-2xl overflow-hidden group">
                ...
                <div className="absolute bottom-4 right-4 bg-slate-950/90 backdrop-blur-md border border-red-900/60 px-3 py-2 rounded-xl text-xs flex items-center gap-2 shadow-lg">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-red-400">ECE & CSE Departments</p>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-2xl mx-auto mb-10">
          <h2 className="text-2xl sm:text-3xl font-extrabold text-white">Faculty Mentors & Student Leadership</h2>
          <p className="text-xs sm:text-sm text-slate-400 mt-2">
            Mentored by experienced LNJPIT faculty professors and spearheaded by dedicated student leads.
          </p>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {(faculty || []).slice(0, 2).map(f => (
            <div key={f.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-center space-y-3">
              <img src={f.photo} alt={f.name} className="w-20 h-20 rounded-full object-cover mx-auto ring-2 ring-red-600/50" />
              <div>
                <h3 className="font-bold text-white text-base">{f.name}</h3>
                <p className="text-xs text-red-400 font-medium">{f.designation}</p>
          ...
          {(coordinators || []).slice(0, 1).map(c => (
            <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 text-center space-y-3">
              <img src={c.photo} alt={c.name} className="w-20 h-20 rounded-full object-cover mx-auto ring-2 ring-amber-500/50" />
              <div>
                <h3 className="font-bold text-white text-base">{c.name}</h3>
                <p className="text-xs text-amber-400 font-medium">{c.position}</p>
                <p className="text-[11px] text-slate-400">{c.branch} ({c.year})</p>
              </div>
